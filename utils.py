...
def qqplot(x1, x2, names, ax=None, quantiles=1000, title='', logscale=False):
    if ax is None:
        fig, ax = plt.subplots()
    if isinstance(ax,str) and ax == 'interactive':
        ax = InteractiveFigure().get_axes()
    if logscale:
        x1 = np.log10(1 + np.abs(x1)) * np.sign(x1)
        x2 = np.log10(1 + np.abs(x2)) * np.sign(x2)

    M = max(np.max(np.abs(x1)),np.max(np.abs(x2)))
    dist1 = dist(x1, 100*np.arange(quantiles+1)/quantiles)
    dist2 = dist(x2, 100*np.arange(quantiles+1)/quantiles)
    ax.plot((-M,M), (-M,M), 'k--')
    ax.plot(dist1[2:], dist2[2:], 'b.-')
    ax.plot((dist1[1],), (dist2[1],), 'ro')
    dist1 = dist(x1, 10*np.arange(11))
    dist2 = dist(x2, 10*np.arange(11))
    for i in range(11):
    ...
    ax.set_ylabel(('Log-' if logscale else '')+names[1]+
                  f' ({counter_to_str(len(x2)):s} samples)', fontsize=12)
    ax.set_title(title, fontsize=14)
    draw()

def counter_to_str(n):
    return f'{n:d}' if n<1e3 else f'{n/1e3:.2f}K' if n<1e6 else f'{n/1e6:.2f}M'
